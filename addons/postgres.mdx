---
description: "Provision a managed PostgreSQL database on Lizard, connect it to a service by reference, run migrations on deploy, and grow its storage."
---
...
- [Next.js with Managed Postgres](/guides/nextjs-postgres) — migrations, service secrets and a saved-note check after restart.
- [LangGraph with Postgres](/guides/langgraph-postgres) — save an interrupted thread and resume it after restart.
- [Managed Addons](/addons) — naming, references, and the dashboard browsers.
- [Cross-Service References](/variables/references) — reference syntax and resolution.

See [storage and recovery](/platform/storage-and-recovery) for backup and restore planning. Local `lizard run` requires `psql` and a reachable database endpoint; it does not run inside the deployed service.
