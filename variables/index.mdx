---
description: "Set secrets at service or project scope, learn the merge precedence, and see which values reach a build versus the running container."
---
...
See the [`lizard secrets` command reference](/cli/secrets) for every flag.
...
For shared resources like a database, **don't** put the DSN in `--global` — bind it per consumer with a [reference](/variables/references):
...
Most variable changes apply with **no rebuild** — the service restarts to pick them up, which takes a few seconds. The exceptions are build-time values baked into the image:
...
If a value you expected isn't there, see [Troubleshooting → a secret or reference isn't showing up](/variables/troubleshooting/reference-not-applied).
...
See [`lizard run` vs. `lizard ssh`](/cli/run#run-vs-ssh).

## See also

- [Cross-Service References](/variables/references) — read one service's values from another without copy-pasting credentials.
- [`lizard secrets`](/cli/secrets) — the full command reference.
