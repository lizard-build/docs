---
description: "Read one service or addon's values from another with ${{name.KEY}} syntax. How references resolve at deploy time and why they beat copying."
---
...
If a reference isn't showing up the way you expect, see [Troubleshooting → a secret or reference isn't showing up](/variables/troubleshooting/reference-not-applied).
...
Putting a shared DSN in project (`--global`) scope exposes it to *every* service, including ones that don't need it. References give you the same single-source-of-truth rotation while keeping each secret scoped to the services that actually consume it. See [Secret scoping](/variables#scoping).

## See also

- [Variables & Secrets](/variables) — scopes and precedence.
