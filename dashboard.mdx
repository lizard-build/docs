---
description: "What the Lizard dashboard adds to the CLI: service status, deploy timeline, streaming logs, metrics, data browsers, team, and billing."
---
...
Manage service- and project-scoped secrets, with masked values and reference (`${{…}}`) support. See [Variables & Secrets](/variables).

### Domains

Add custom domains, view DNS records, and track verification and TLS status. See [Networking](/networking).
...
Connect the GitHub App, pick repositories, and manage which repos Lizard can build. See [Deploy from GitHub](/deploy/github).

### Settings, team & billing

Project and workspace settings, member invites and roles, resource usage, account credits and optional auto top-up. Standard accounts use [pay as you go](/platform/billing) with no monthly subscription.
