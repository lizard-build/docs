---
description: "Every class and method in the Lizard SDK for JavaScript and Python: Lizard, Sandbox, CodeSandbox, and Volume, with options and return types."
---
...
|---|---|---|---|
| `template` | string | `'base'` | `'base'` or `'code-interpreter-v1'` — the two supported [templates](/sandboxes#templates) |
...
| `volumeName` | string | — | Attach a named [volume](/sandboxes/volumes) from the same project at `/workspace` (`volume_name` in Python) |
...
See [Persistent Volumes](/sandboxes/volumes) for the full walkthrough. Names are unique per project and use lowercase letters, digits and dashes, up to 64 characters.
...
Before relying on pause to retain a session beyond its original deadline, read [known issues](/platform/known-issues#sandbox-pause-and-expiration). Pause keeps guest state in host memory; it is not a durable backup.
