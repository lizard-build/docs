...
Boot a sandbox, run code in it, expose a port, and checkpoint it — with the Lizard SDK. Everything here works the same against the [REST API](/sandboxes/api-reference) if you'd rather not use the SDK.
...
# Optional — override the API base URL (default: https://api.lizard.run)
export LIZARD_API_URL="https://api.lizard.run"
```
...
`getHost(port)` registers a route to that port and returns the hostname. The URL is public and TLS-terminated. Remove it again from the dashboard or the [REST unexpose endpoint](/sandboxes/api-reference#expose-a-port).
...
- **[REST API](/sandboxes/api-reference)** — drive sandboxes from any language, no SDK.
