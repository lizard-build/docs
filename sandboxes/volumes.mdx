---
description: "Attach a volume at /data to keep checkpoints, weights, or a working directory across sandbox restarts. Create, attach, and manage volumes."
---
...
- A volume can only be attached to **one sandbox** at a time. Attempting to attach an already-attached volume returns a conflict.
...
- A paused sandbox still holds its attachment. Persistent Volumes use node-local storage; export data you need to recover after a node failure. See [storage and recovery](/platform/storage-and-recovery).

## See also

- [SDK Reference](/sandboxes/sdk-reference) — every `Volume` method.
- [Quickstart](/sandboxes/quickstart) — attaching a volume at sandbox creation.
