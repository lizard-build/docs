...
| [Coding agent: upload](/guides/deploy-from-coding-agent) | 合格 | `lizard up`、health 200、データベースを使用したレスポンス 200、未知のルート 404; 更新された Postgres 行はサービス再起動後も保持された |
| [Coding agent: GitHub](/guides/deploy-from-coding-agent) | 合格 | リポジトリは `--no-deploy` で接続; ブランチ、ルートディレクトリ、環境は `redeploy` の前に設定; 公開アプリは同じ既存の Postgres 行を読み取った |
| [Redis worker](/deploy/workers) | 合格 | Port 0; enqueue と result; result は再起動後も保持; 再起動後の新しいジョブ; 完了していない processing エントリは起動時に復旧 |
...
CLI アーカイブと failed-build の exit-code 制限については [framework test results](/framework-guides/validation) の別の 16 レシピの framework バッチと、[known issues](/platform/known-issues) を参照してください。
