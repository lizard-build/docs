...
一時停止するとゲストの vCPU は凍結され、現在の状態はホストメモリに保持されます。永続的なスナップショットは書き込みません。元の期限を超えてセッションを保持する前に、[一時停止と期限切れの問題](/platform/known-issues#sandbox-pause-and-expiration) を確認してください。
...
- **[コード インタープリタ](/sandboxes/code-interpreter)** — 状態を保持するマルチ言語コードを実行。
- **[Persistent Volumes](/sandboxes/volumes)** — 単一のサンドボックスより長く存続するストレージをアタッチ。
