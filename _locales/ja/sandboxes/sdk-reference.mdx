---
description: "JavaScript と Python 向け Lizard SDK のすべてのクラスとメソッド: Lizard、サンドボックス、CodeSandbox、ボリューム。オプションと戻り値の型も含みます。"
---
...
`@lizard-build/sdk` (JS/TS) と `lizard-sdk` (Python) パッケージは、ほとんどのエージェント、アプリ、スクリプトが sandbox を操作するために使用します。このページではすべてのクラスとメソッドを一覧化しています。手順の説明は [クイックスタート](/sandboxes/quickstart) を参照してください。
...
状態を保持するカーネルで `Sandbox` を拡張したものです。完全な手順は [コード インタープリタ](/sandboxes/code-interpreter) を参照してください。
...
- [コード インタープリタ](/sandboxes/code-interpreter) — `CodeSandbox` ガイド。
