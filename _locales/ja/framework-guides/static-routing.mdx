...
生成ページのポリシーは、アプリにそれらのルートファイルがある場合にのみ使用してください。たとえば、SvelteKit の SPA フォールバックには、その専用のルーティングポリシーが必要です。[Next.js静的エクスポート](/framework-guides/nextjs/static-export) には、trailing-slash レイアウトを含む完全な手順があります。
