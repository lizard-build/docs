...
|-------|---------|-----------|------------|
| **Service**（デフォルト） | `lizard secrets set K=v --service <svc>` | `secrets.services[<svc>]` | そのサービスのみ |
| **Project**（グローバル） | `lizard secrets set K=v --global` | `secrets.shared` | プロジェクト内のすべてのサービス |
...
[Build Pipeline → rebuild triggers](/concepts/build-pipeline#what-triggers-a-rebuild) を参照してください。
...
- [Cross-Service References](/variables/references) — 認証情報をコピー＆ペーストせずに、あるサービスから別のサービスの値を読み取ります。
