...
Das erste Redis-Addon heißt `redis`, daher funktioniert `${{redis.REDIS_URL}}` sofort.
...
Der Verweis wird beim Deploy aufgelöst und mit den Zugangsdaten des Addons rotiert.
...
`lizard run` startet den Befehl lokal mit der ausgewählten Service-Umgebung. Installieren Sie `redis-cli` lokal und prüfen Sie, dass der Endpoint erreichbar ist.
