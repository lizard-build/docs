---
description: "Was das Lizard-Dashboard zusätzlich zur CLI bietet: Service-Status, Deploy-Zeitleiste, Live-Logs, Metriken, Datenbrowser, Team und Abrechnung."
---
...
Eine Zeitleiste aller Deploys mit Build-Logs pro Deploy und einer Detailansicht (Commit, Status, Replikate). Das ist das visuelle Gegenstück zu `lizard events` + `lizard logs --build`.
...
Jedes Addon enthält einen Datenbrowser:

- **Postgres** — ein SQL-/Tabelleneditor zum Abfragen und Bearbeiten von Daten.
- **Redis** — ein Key/Value-Browser.
- **S3** — ein Bucket- und Objektbrowser mit Upload/Download.

### GitHub-Integration

Verbinden Sie die GitHub-App, wählen Sie Repositories aus und verwalten Sie, welche Repos Lizard bauen darf. Siehe [Deploy from GitHub](/deploy/github).
...
|------|-----|----------------|
| Deploy-Verlauf | `lizard events` | Deployments |
...
| Postgres/Redis/S3 durchsuchen | `lizard run` / `lizard ssh` | Addon-Browser |
