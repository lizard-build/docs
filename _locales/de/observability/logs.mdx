---
description: "Laufzeit-Logs streamen, Build-Logs nach einem fehlgeschlagenen Deploy lesen und den Log-Tail rund um einen Neustart abrufen – über die CLI oder das Dashboard."
---
...
Das ist das Erste, was du prüfen solltest, wenn ein Deploy fehlschlägt — lies die Logs, behebe die Ursache und dann `lizard redeploy`. Siehe [Build-Pipeline](/concepts/build-pipeline).
...
Das Dashboard (`lizard open`) streamt Laufzeit- und Build-Logs mit Such- und Level-Filtern und zeigt die Build-Ausgabe pro Deployment in der Ansicht **Deployments**.
...
- [Ereignisse & Verlauf](/observability/events) — Deploy-Verlauf und Replik-Status.
- [Deploy aus Claude Code](https://lizard.build/blog/deploy-from-claude-code) — Logs, die dafür geschrieben sind, von einem Agenten gelesen zu werden, nicht nur von dir.
