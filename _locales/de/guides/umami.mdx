...
Erstelle vor dem Upgrade ein Backup von PostgreSQL und lies die Release Notes. Ändere das Image-Tag im lokalen Dockerfile und führe dann `lizard up --service umami --port 3000` erneut aus. Bei diesem Upload-basierten Setup erstellt `lizard redeploy` die zuletzt hochgeladenen Dateien neu; lokale Änderungen werden nicht hochgeladen.

Siehe [Lizard's PostgreSQL guide](https://lizard.build/docs/addons/postgres/) für den Datenbankzugriff und [storage and recovery](https://lizard.build/docs/platform/storage-and-recovery/) für die Backup-Planung.
