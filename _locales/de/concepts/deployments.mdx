---
description: "Der Deploy-Lebenszyklus auf Lizard: was einen Build auslöst, wie der Traffic auf fehlerfreie Replikate umgeschaltet wird und wie sich Neustarts von Redeploys unterscheiden."
---
...
2. **Vor dem Deploy** — wenn `preDeployCommand` gesetzt ist, wird es einmal ausgeführt (z. B. Migrationen).
3. **Start** — Lizard startet die Anwendung für ihre konfigurierte Laufzeitumgebung.
4. **Health-Check** — die Plattform prüft, ob der Port der App erreichbar ist (entfällt bei [Workers](/deploy/workers)).
5. **Prüfen** — prüfen Sie den Release-Status und rufen Sie die Anwendungs-URL auf. Die Erreichbarkeit des Ports ist kein vollständiger Health-Check der Anwendung.

<span id="streaming-output" />

## Streaming-Ausgabe

Wenn Sie ein nicht detachdtes `lizard up` (oder `redeploy`) ausführen, werden Build-Logs live gestreamt. Mit `--json` erhalten Sie ein JSON-Ereignis pro Zeile:
...
endet mit `deployed`, `failed` oder `deploying`. Verwenden Sie `--detach`, um das Deploy zu starten und sofort zurückzukehren, ohne Streaming.
...
Im [Dashboard](/dashboard) zeigt die Ansicht **Deployments** die vollständige Zeitleiste, Build-Logs pro Deployment und einen Detailbereich für jedes Release.
