...
| **Project** (global) | `lizard secrets set K=v --global` | `secrets.shared` | jeder Service im Projekt |
...
- Änderungen an `VITE_*` und `NEXT_PUBLIC_*` **erzwingen beim nächsten Deploy einen Rebuild**.
...
- [Cross-Service References](/variables/references) — lesen Sie die Werte eines Service aus einem anderen, ohne Anmeldedaten per Copy-paste zu duplizieren.
