...
| `sandbox.pause()` | Guest-vCPUs stoppen und den Zustand im Host-Speicher behalten. Lesen Sie den [Lifecycle-Status](#lifecycle-status), bevor Sie sich auf eine Pause über die ursprüngliche Frist hinaus verlassen. |
...
| `onResult` | Wird für jedes Rich Result aufgerufen (Werte, erzeugte Bilder/Diagramme). |
