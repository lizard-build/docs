...
| `volumeId` | string | — | Adjunta un [volume](/sandboxes/volumes) en `/data` |
...
Consulta [Persistent Volumes](/sandboxes/volumes) para ver el recorrido completo.

| Miembro | Descripción |
|---|---|
| `Volume.create(projectId, name, options)` | Crea un volume. `options.sizeGb` usa por defecto `5`. |
| `Volume.list(projectId)` | Lista los volúmenes de un proyecto → `[{ id, name, sizeGb, status, attachedTo, … }]`. |
| `Volume.get(projectId, volumeId)` | Obtiene un identificador de un volume existente. |
| `volume.getInfo(projectId)` | Obtiene metadatos y el estado del adjunto. |
| `volume.delete(projectId)` | Elimina un volume: primero debe estar desacoplado. |
