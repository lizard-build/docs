...
Esto crea un servicio con origen `github`, clona el repositorio, detecta automáticamente el stack ([lizardpack](/concepts/build-pipeline)), hace el build y devuelve una URL activa. Flags útiles:
...
- [Build Pipeline](/concepts/build-pipeline) — cómo se detecta y se hace build de tu stack.
- [Deploy from Local Code](/deploy/upload) — cuando no tienes un remoto.
- [`lizard git`](/cli/git) — la referencia completa de comandos para `connect`, `status` y `checkout`.
- [Where to deploy an app your AI IDE built](https://lizard.build/blog/google-antigravity-how-to-deploy-your-app-to-production#where-to-deploy-an-app-your-ai-ide-built) — cómo apuntar esto a un repositorio que escribió un agente, y cuánto cuesta.
