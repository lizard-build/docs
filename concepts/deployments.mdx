...
A **deployment** is one build-and-release of a service. Every deploy produces a build, starts new replicas in fresh pods, and shifts traffic once they're healthy. Rolling replicas without touching a server is what you pay a [container as a service](https://lizard.build/blog/container-as-a-service) for; the blog prices nine of them.
...
3. **Start** — new replicas start in fresh pods.
