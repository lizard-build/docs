---
description: "How Lizard organizes work into workspaces, projects, and services, plus managed addons, cross-resource references, and injected variables."
---
...
Each running service runs as its own **isolated pod** with a default-deny network policy, a generated `<name>.<region>.onlizard.com` domain, and automatic TLS. That split — you bring the container, the platform runs it — is [container as a service](https://lizard.build/blog/container-as-a-service), and the blog covers what it still leaves you holding. Inspect and manage services with:
...
Service configuration fields are **flat** and map 1:1 to the wire schema (e.g. `repoUrl`, `branch`, `buildCommand`, `startCommand`, `containerPort`, `rootDirectory`). There is no `build.*` / `deploy.*` nesting. See [`lizard service`](/cli/service) for the full list.
...
See [Variables & Secrets](/variables) for how these interact with your own variables and the full precedence order.
