...
Set `url` in `docusaurus.config.js` to your public HTTPS origin and rebuild after the hostname changes. Check `/docs/guide/`, `/probe.txt`, and a missing path.
...
```sh
lizard init --name docusaurus-example
...
See the [full docusaurus guide](https://lizard.build/docs/framework-guides/docusaurus) for configuration and limits. Dependencies use the versions tested with this example; review updates before adopting them in your app.
