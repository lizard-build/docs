...
Set `baseURL` in `hugo.toml` to your public HTTPS URL, including the trailing slash, and rebuild after the hostname changes. This example uses local layouts and has no theme or Hugo Modules dependency. Check `/guide/`, `/probe.txt`, and a missing path.
...
See the [full hugo guide](https://lizard.build/docs/framework-guides/hugo) for configuration and limits. Dependencies use the versions tested with this example; review updates before adopting them in your app.
